...
Платформата поддържа следните тестови команди:

```bash
npm test          # стартира Vitest за всички frontend unit тестове
npm run test:e2e  # стартира Playwright e2e тестове
pytest backend/tests/ --cov=backend --cov-report=xml  # стартира backend тестовете
```
