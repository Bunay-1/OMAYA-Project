...
### Добавени
- Актуализация до версия 3.1.5.
- Преминаване към реална аутентификация чрез база данни (TimescaleDB/PostgreSQL).
- Интегрирани реални AI/ML модели (TensorFlow LSTM и SKLearn RUL) на мястото на mock имплементациите.
- Нова таблица `machines` в базата данни за управление на метаданни на активите.
- Пълна интеграция на телеметрията с TimescaleDB (съхранение на хронологични данни).
- Подобрена сигурност чрез премахване на mock потребители и въвеждане на реална RBAC система.

## [3.1.4] - 2026-06-25

### Добавени
- Актуализация до версия 3.1.4.
- Внедрена RAG (Retrieval-Augmented Generation) система за интелигентно извличане на информация от документи.
- Поддръжка на широк спектър от файлови формати: .pdf, .docx, .pptx, .xlsx, .csv, .json, .xml, .txt, .md и .odt.
- Нов UI компонент `RAGDashboard.tsx` за търсене в базата знания и управление на индекса.
- Интеграция с FAISS за векторно съхранение и HuggingFace ембединги (all-MiniLM-L6-v2).
- API ендпоинти за заявки към RAG, ре-индексиране и статистика.
- Добавена поддръжка за .odt файлове чрез библиотеката `odfpy`.

## [3.1.3] - 2026-06-25

### Добавени
- Актуализация до версия 3.1.3.
- Внедрен нов модул за визуална инспекция (Visual Inspection) с YOLO.
- Поддръжка за разпознаване на качествени и некачествени детайли в реално време.
- Нов UI компонент `VisualInspection.tsx` и интеграция в Sidebar.
- Нови API ендпоинти за обработка на изображения чрез YOLO.

## [3.1.2] - 2026-06-25

### Добавени
- Актуализация до версия 3.1.2.
- Корекция на URL за клониране в документацията.
- Подобрена сигурност чрез премахване на hardcoded пароли в Docker конфигурациите.
- Ревизиран статус на функционалностите в README за по-голяма прозрачност.
- Обновени баджове и статистики за зрялост на платформата.

## [3.1.1] - 2026-06-25

### Добавени
- Актуализация до версия 3.1.1.
- Подобрено тестово покритие за PLC адаптери (Siemens S7, Fanuc, Modbus).
- Внедрен HIL (Hardware-in-the-Loop) симулатор за индустриални протоколи.
- Автоматично ретрениране при открит Model Drift (PSI > 0.2).
- Rate limiting за WebSocket съобщения.
- Интеграция на Kafka Schema Registry за управление на версиите на съобщенията.
- Environment guard във фронтенда за защита на производствената среда от mock данни.
...
- Внедрен механизъм за динамично зареждане (lazy loading) на `mockData.ts`, за да се избегне включването на тестови данни в производствения билд.
